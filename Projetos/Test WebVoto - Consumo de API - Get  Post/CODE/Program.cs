        ...
        foreach (var job in jobRetorno)
        {
            if (job.Type == "DecodeStrand")
            {
                ...
                using (var clientDecodeStrand = new HttpClient()) //POST - decode
                {
                    ...
                    var responseDecodeStrand = await client.PostAsJsonAsync($"/api/dna/jobs/{job.Id}/decode", new JobsReturn(strandDecode, "", false));
            ...
            else if (job.Type == "EncodeStrand")
            {
                ...
                using (var clientEncodeStrand = new HttpClient()) //POST - encode
                {
                    ...
                    var responseEncodeStrand = await client.PostAsJsonAsync($"/api/dna/jobs/{job.Id}/encode", new JobsReturn("", strandEncode, false));
            ...
            else if (job.Type == "CheckGene")
            {
                ...
                using (var clientGeneEncode = new HttpClient()) //POST - gene
                {
                    ...
                    var responseGeneEncode = await client.PostAsJsonAsync($"/api/dna/jobs/{job.Id}/gene", new JobsReturn("", "", isActvated));
